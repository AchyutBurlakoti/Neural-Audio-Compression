...
def torch_ged_custom(wav_fake, wav_real):
    """Multi-scale spectrogram-based generalized energy distance.

    Args:
      wav_fake: Float tensors of shape [batch, time, 1].
        Generated audio samples conditional on a set of linguistic features.
        Real (data) audio samples corresponding to the same features.
    ...
    specs_fake, specs_real = torch_calc_spectrograms(
        waves=[wav_fake, wav_real],
        window_lengths=[2**i for i in range(6, 12)])

    dist_real_fake = torch_sum_spectral_dist(specs_real, specs_fake)

    return dist_real_fake
...
#     input = torch.randn(1, 22050)
#     output = torch.randn(1, 24000)
#     loss = torch_ged_custom(input, output)
